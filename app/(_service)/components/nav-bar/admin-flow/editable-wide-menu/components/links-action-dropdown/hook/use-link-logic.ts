...
export function useLinkLogic({
  ...
  const handleLinkToggle = useCallback((linkType: LinkType) => {
    ...
    const newConfiguration = updateLinkConfiguration(linkConfiguration, linkType, newState);
    ...
    console.log("LINK TOGGLED", singlePage.id, linkType, `${currentState} -> ${newState}`);
  }, [singlePage.id, categoryTitle, setCategories, isInteractive, linkConfiguration]);
