...
import { useModal } from "@/app/(_service)/contexts/modal-context";
...
function useEnvStatus() {
  ...
  return { siteNamePresent, logoPresent, siteNameFromEnv, logoFromEnv, loading };
}
...
export function HomeActionsMenu({
  ...
  const { siteNamePresent, logoPresent, siteNameFromEnv, logoFromEnv } = useEnvStatus();
  ...
  const handlePageBodyClick = () => {
    if (isBlocked) {
      toast.warning("Please save changes before entering page body editing mode", {
        description: "You have unsaved changes in the navigation menu. Save them first to avoid data conflicts.",
  ...
  const handleSiteNameInfo = () => {
    if (!siteNamePresent) {
      toast.error("Failed to load site name configuration.", {
        description: "Check your environment setup and try refreshing the page.",
        
      });
    } else if (!siteNameFromEnv) {
      toast.info("Using default site name.", {
        description: "Set SITE_NAME environment variable to customize: 'Aifa Blog Starter for GitHub and AI'",
  ...
  const handleLogoInfo = () => {
    if (!logoPresent) {
      ...
      toast.info("Using default logo.", {
        description: "Set SITE_LOGO_URL environment variable to customize. **Use only the link you get when uploading the image. The image must be saved with the name Logo.** Use minimum image size, no more than 20kB.",
