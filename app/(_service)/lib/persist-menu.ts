...
export function isGitHubError(result: PersistMenuResult): boolean {
  return isErrorResponse(result) && result.status === OperationStatus.GITHUB_API_ERROR;
}
...
export function isFilesystemError(result: PersistMenuResult): boolean {
  return isErrorResponse(result) && result.status === OperationStatus.FILESYSTEM_ERROR;
}
...
export function isValidationError(result: PersistMenuResult): boolean {
  return isErrorResponse(result) && result.status === OperationStatus.VALIDATION_ERROR;
}
...
export function shouldRetry(result: PersistMenuResult): boolean {
  ...
  if (isErrorResponse(result)) {
    ...
    return result.status === OperationStatus.GITHUB_API_ERROR || 
