...
export function useImages(initialImages?: PageImages[]) {
  ...
  const validateImagesBeforeAdd = useCallback((): boolean => {
    const hasIncompleteImages = imagesList.some(image => 
      (!image.alt?.trim() || !image.href?.trim())
    );
    
    if (hasIncompleteImages) {
      setValidationError("Please fill both Alt description and Image URL for all existing images before adding new ones");
      return false;
    }
  ...
  const handleImageChange = useCallback((index: number, field: 'alt' | 'href', value: string) => {
    setImagesList(prev => 
      prev.map((image, i) => 
