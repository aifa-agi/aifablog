...
interface ImagesInputProps {
  images: {
    ...
    handleImageChange: (index: number, field: 'alt' | 'href', value: string) => void;
  };
  isLoading?: boolean;
}
...
export function ImagesInput({ images, isLoading = false }: ImagesInputProps) {
  ...
  const canAddMore = images.imagesList.every(image =>
    image.alt?.trim() && image.href?.trim()
  );
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-2 text-xs text-muted-foreground">
        <p>
          <strong>Use maximally detailed descriptions for images so artificial intelligence can make decisions about which area of the site this image will be most useful.</strong>
        </p>
        <p>
          Both Alt description and Image URL are required. Images without both fields will be automatically removed when saving.
        </p>
      </div>
    </div>
  );
