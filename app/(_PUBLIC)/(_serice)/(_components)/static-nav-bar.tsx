  ...
  return (
    <>
      <div className="relative">
        <div className="flex items-center px-4 h-[56px]" style={{ minHeight: HEADER_HEIGHT, maxHeight: HEADER_HEIGHT }}>
          {isLargeScreen ? (
            <Button onClick={handleButtonClick} size="sm" className="flex items-center gap-2 whitespace-nowrap px-4">
              <span>{isOpen ? 'Close bar menu' : 'Open bar menu'}</span>
              <ChevronDown className={`w-4 h-4 transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`} />
            </Button>
          ) : (
            <Button onClick={handleButtonClick} className="flex items-center justify-center px-2" aria-label={isOpen ? 'Close bar menu' : 'Open bar menu'}>
