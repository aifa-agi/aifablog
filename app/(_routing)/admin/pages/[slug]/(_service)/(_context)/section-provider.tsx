...
interface SectionContextValue {
  ...
  loading: Record<string, boolean>;
  errors: Record<string, string | null>;
  loadSections: (href: string, force?: boolean) => Promise<ExtendedSection[] | null>;
...
export function SectionProvider({ 
  ...
  const isCacheValid = useCallback((href: string): boolean => {
...
export function usePageSections(href?: string) {
  ...
  const update = React.useCallback((sections: ExtendedSection[]) => {
    if (!href) return;
...
export function useMultiPageSections(hrefs: string[]) {
  ...
  return {
    ...
    totalSections: Object.values(sectionsMap).reduce((sum, sections) => sum + (sections?.length || 0), 0),
  };
}
