...
// @/app/(_routing)/admin/pages/[slug]/(_service)/(_components)/admin-pages/hooks/use-section-operations.ts
"use client";
...
export const useSectionOperations = (pageHref?: string) => {
  ...
  const handleReloadSections = useCallback(async () => {
    if (!pageHref) return;
